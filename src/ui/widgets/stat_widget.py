...
from world.systems.combat.entity_stats import EntityStats, StatDefinition
...
class StatWidget(gui.UIAnchorLayout):
    ...
    def update_stats(self, stats: EntityStats):
        for widget in self.box1.children:
            self._update_single_widget(widget, stats)
            
        for widget in self.box2.children:
            self._update_single_widget(widget, stats)
            
    def _update_single_widget(self, widget, stats: EntityStats):
        stat_name = EntityStats.resolve(widget.widget_stat)
